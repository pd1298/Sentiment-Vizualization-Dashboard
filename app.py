...
st.markdown("This application is Dashboard to analyze the Sentiments of Tweets")
st.sidebar.markdown("The application is a streamlit dashboard to analyze the Sentiment")

DATA_URL = (r"Tweets_main.csv")
...
if not st.sidebar.checkbox("Hide", True):
    ...
    if select == "Histogram":
        ...
        fig = px.pie(sentiment_count, values = 'Tweets', color = 'Tweets',names = 'Sentiment')
        st.plotly_chart(fig)
...
if not st.sidebar.checkbox("Close", True , key = '1'):
     ...
     st.markdown("%i tweets between %i:00 an %i:00" % (len(modified_data), hour, (hour+1)%24))
...
choice = st.sidebar.multiselect('Pick airlines', ('US Airways', 'United', 'American', 'Southwest', 'Delta' , 'Virgin'))
